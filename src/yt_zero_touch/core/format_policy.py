...
# Plain height-capped selectors carry no codec filter, so codec preference lives
# entirely here instead — an avc1-first filter in the format selector would cap
# "Best"/"4K" at 1080p, since YouTube only serves H.264 up to 1080p and a valid
# avc1 match would win before the real 4K/VP9/AV1 streams are ever considered.
# Ranking "res"/"fps" ahead of the codec preference below lets 1440p/4K actually
# pick the higher-res VP9/AV1 stream instead of being capped at a 1080p avc1 match.
DONT_CAP_RESOLUTION = ["res", "fps"]

# Within a resolution tier, prefer H.264 — keeps <=1080p output Premiere-native
# (H.264) without affecting which resolution tier gets picked (that's already
# decided by DONT_CAP_RESOLUTION above).
...
# stays byte-identical to the list this module replaces.
AUDIO_TRACK_QUALITY_TIEBREAKERS = ["channels", "abr"]

# The actual yt-dlp `format_sort` value — wired straight into ydl_opts.
FORMAT_SORT = [
